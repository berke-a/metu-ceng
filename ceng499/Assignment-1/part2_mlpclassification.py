...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    output = softmax(output_layer+b2)
...
w1 = torch.from_numpy(np.random.normal(0, 1, 48).astype(
    np.float32).reshape((3, 16))).requires_grad_()
...
b1 = torch.from_numpy(np.random.normal(0, 1, 1).astype(
    np.float32).reshape((1, 1))).requires_grad_()
...
w2 = torch.from_numpy(np.random.normal(0, 1, 48).astype(
    np.float32).reshape((16, 3))).requires_grad_()
...
b2 = torch.from_numpy(np.random.normal(0, 1, 1).astype(
    np.float32).reshape((1, 1))).requires_grad_()
...
for iteration in range(1, ITERATION+1):
    ...
    train_mean_crossentropy_loss = torch.mean(
        cross_entropy(train_predictions, train_label))
    ...
    with torch.no_grad():
        ...
        validation_mean_crossentropy_loss = torch.mean(
            cross_entropy(validation_predictions, validation_label))
